...
const JobDetail = ({ getJob, applyToJob, job: { job, loading, myApplications }, auth }) => {
  const { id } = useParams();
  ...
  return (
    <div className="min-h-screen bg-black pt-24 pb-16 px-4">
      ...
      <div className="relative max-w-4xl mx-auto">
        <Link to="/jobs" className="inline-flex items-center gap-2 text-white/50 hover:text-white text-sm transition-colors mb-8">
          ...
          <div className="lg:col-span-2 space-y-6 animate-fade-in">
            ...
            <div className="glass rounded-3xl p-6 border border-white/10">
              <div className="flex items-start gap-4 mb-4">
                {org?.logo ? (
                  <img src={org.logo} alt={org.name} className="w-16 h-16 rounded-2xl object-contain bg-white/5 p-1 shrink-0" />
                ) : (
                ...
                <div>
                  ...
                  <p className="text-white/60 text-sm mt-0.5">{org?.name} · {org?.industry}</p>
                  {org?.website && (
                    <a href={org.website} target="_blank" rel="noreferrer" className="text-indigo-400 text-xs hover:text-indigo-300 transition-colors">
                      {org.website}
                    </a>
                  )}
                </div>
              ...
              <div className="flex flex-wrap gap-2">
                <span className="px-3 py-1.5 rounded-full text-xs font-medium bg-indigo-500/10 text-indigo-400 border border-indigo-500/20">{job.jobType}</span>
                <span className="px-3 py-1.5 rounded-full text-xs font-medium bg-white/5 text-white/60 border border-white/10">{job.workMode}</span>
                <span className="px-3 py-1.5 rounded-full text-xs font-medium bg-white/5 text-white/60 border border-white/10">📍 {job.location}</span>
                {salaryStr && <span className="px-3 py-1.5 rounded-full text-xs font-medium bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">💰 {salaryStr}</span>}
              </div>
            </div>
            ...
            <div className="glass rounded-3xl p-6 border border-white/10">
              ...
              <div className="flex flex-wrap gap-2">
                {job.skills.map((s) => (
                  <span key={s} className="px-3 py-1.5 rounded-full bg-indigo-500/10 text-indigo-300 border border-indigo-500/20 text-sm">
                    {s}
                  </span>
                ))}
              </div>
            ...
            {showApply && (
              <div className="glass rounded-3xl p-6 border border-white/10 animate-slide-up">
                <h2 className="text-white font-semibold mb-4">Your Application</h2>
                <form onSubmit={onSubmit} className="space-y-4">
                  <div className="flex flex-col gap-1.5">
                    <label className="text-xs font-medium text-white/50 uppercase tracking-widest">Cover Letter *</label>
                    <textarea
                  ...
                  <div className="flex flex-col gap-1.5">
                    <label className="text-xs font-medium text-white/50 uppercase tracking-widest">Resume URL (optional)</label>
                    <input
                      type="url"
          ...
          <div className="space-y-4 animate-fade-in">
            ...
            <div className="glass rounded-3xl p-5 border border-white/10 sticky top-24">
              {!auth.isAuthenticated ? (
                <div className="text-center space-y-3">
                  ...
                  <Link to="/login" className="block w-full py-3 rounded-xl bg-gradient-to-r from-indigo-600 to-blue-600 text-white font-semibold text-sm text-center transition-all">
                    Sign in
                  </Link>
                </div>
              ) : hasApplied || submitted ? (
                <div className="text-center py-3">
                  <div className="text-emerald-400 font-medium text-sm">✓ Applied</div>
                  <p className="text-white/30 text-xs mt-1">You've applied to this job</p>
                </div>
              ) : job.status !== 'Open' ? (
                <div className="text-center py-3">
                  <div className="text-red-400 font-medium text-sm">Applications Closed</div>
                </div>
              ) : (
                <button
            ...
            <div className="glass rounded-3xl p-5 border border-white/10 space-y-3">
              ...
              {[
                ...
                { label: 'Deadline', value: job.applicationDeadline ? new Date(job.applicationDeadline).toLocaleDateString() : 'Not specified' },
                { label: 'Posted', value: new Date(job.datePosted).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) }
              ].map(({ label, value }) => (
            ...
            {org && (
              <div className="glass rounded-3xl p-5 border border-white/10 space-y-2">
                <h3 className="text-white font-semibold text-sm">About {org.name}</h3>
                {org.description && <p className="text-white/50 text-xs leading-relaxed">{org.description}</p>}
                <div className="space-y-1.5 pt-1">
                  {[
                    { label: 'Industry', value: org.industry },
                    { label: 'Size', value: org.employeesCount && `${org.employeesCount} employees` },
